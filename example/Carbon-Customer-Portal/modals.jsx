...
function AppFormScreen({ mode, app, onClose, onSave }) {
  ...
  //   - new app: goes into the local pool as "not-published". The ISV can
  //     submit it from the App Detail page when ready.
  //   - edit (metadata change on a published app): queues a re-review.
  //   - edit (rejected): re-submits to Admin.
  //   - otherwise: no review impact.
  const savePreview = (() => {
    if (!isEdit) return { kind: "new", label: hasISO
      ? "Will be created in your local pool. Submit to public pool review when ready."
      : "Will be created in your local pool. You can submit it for public pool review when ready." };
    const metaChanged = app && (name !== app.name || category !== app.category || description !== app.description);
    if (metaChanged && app?.status === "published") return { kind: "recheck", label: "Name / category / description changed — saving queues a fresh Admin review." };
    if (metaChanged && app?.status === "rejected") return { kind: "resubmit", label: "Will be re-submitted to Admin on save." };
    return { kind: "none", label: "No review changes needed." };
  })();
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <window.Button primary disabled={!canSave} icon={isEdit ? "check" : "plus"}
          onClick={() => { onSave && onSave({ name, pkg, category, description, devices, orientations }); onClose(); }}>
