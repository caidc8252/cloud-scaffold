...
function PublishWizardScreen({ app, onClose, onPublish, toast }) {
  ...
  const canNext = (() => {
    ...
    if (step === 1) return scanState === "done";
    if (step === 2) return versionNotes.trim().length > 5;
    if (step === 3) return scopeMode !== "whitelist" || whitelist.size > 0;
    return true;
  })();
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={onClose} style={{ color: "var(--fg-tertiary)", padding: 2 }} title="Back">
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {app && (
              <div className="mono" style={{ marginTop: 2, fontSize: 11.5, color: "var(--fg-tertiary)" }}>
                {app.package}
              </div>
            )}
          </div>
              ...
              <div key={s.id} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <div style={{
                    ...
                    background: done ? "var(--accent-gradient)" : active ? "var(--bg-surface)" : "transparent",
                    border: "1px solid", borderColor: active ? "var(--accent-500)" : done ? "transparent" : "var(--border-default)",
                    color: done ? "var(--fg-on-accent)" : active ? "var(--accent-600)" : "var(--fg-tertiary)",
                  ...
                  <span style={{
                    ...
                    color: active ? "var(--fg-primary)" : done ? "var(--fg-secondary)" : "var(--fg-tertiary)",
                    whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
                  }}>{s.label}</span>
                </div>
                <div style={{
                  height: 2, borderRadius: 1,
                  background: done ? "var(--accent-gradient)" : active ? "var(--accent-200)" : "var(--border-subtle)",
                }} />
              </div>
            );
          })}
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: "auto", background: "var(--bg-app)" }}>
        <div style={{ maxWidth: 880, margin: "0 auto", padding: "24px 24px 32px" }}>
          ...
          {step === 1 && <StepScan state={scanState} pct={scanPct} counts={scanCounts} findings={findings} />}
          {step === 2 && <StepInfo notes={versionNotes} setNotes={setVersionNotes} screenshots={screenshots} setScreenshots={setScreenshots} />}
          {step === 3 && <StepScope subscriberIds={baseSubscribers}
          ...
          {step === 4 && <StepReview app={app} parsed={parsed} counts={scanCounts}
      ...
      <footer style={{
        ...
        <div style={{ flex: 1, fontSize: 11.5, color: "var(--fg-tertiary)" }}>
          Step <span className="mono" style={{ color: "var(--fg-primary)", fontWeight: 500 }}>{step + 1}</span> of {steps.length}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {state === "idle" && (
        ...
        onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--accent-500)"; e.currentTarget.style.background = "var(--accent-50)"; }}
        onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--border-default)"; e.currentTarget.style.background = "var(--bg-sunken)"; }}>
          <div style={{ width: 44, height: 44, borderRadius: 10,
            background: "var(--bg-surface)", border: "1px solid var(--border-default)",
            color: "var(--fg-secondary)", display: "grid", placeItems: "center" }}>
        ...
        <div style={{ padding: "24px", background: "var(--bg-surface)",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: "var(--fg-tertiary)" }}>Uploading… <span className="mono">{Math.round(pct)}%</span></div>
            </div>
            <span className="mono" style={{ fontSize: 11.5, color: "var(--fg-tertiary)" }}>28.4&nbsp;MB</span>
          </div>
          <div style={{ marginTop: 14, height: 4, borderRadius: 999, background: "var(--bg-sunken)", overflow: "hidden" }}>
            <div style={{ width: `${pct}%`, height: "100%", background: "var(--accent-gradient)", transition: "width .1s" }} />
          </div>
        </div>
      )}
      ...
      {state === "parsed" && parsed && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ padding: "12px 16px",
            ...
            border: "1px solid", borderColor: "color-mix(in oklab, var(--success) 25%, transparent)",
            display: "flex", alignItems: "center", gap: 10 }}>
            <window.Ico name="check" size={14} style={{ color: "var(--success)" }} stroke={2.5} />
            <div style={{ flex: 1, fontSize: 12.5, fontWeight: 500, color: "var(--success)" }}>
          ...
          {app && parsed.package !== app.package && (
            <div style={{ padding: 12, background: "var(--danger-bg)", borderRadius: 8,
        ...
        <>
          <div style={{ padding: 14, background: "var(--bg-surface)",
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 12, color: "var(--fg-tertiary)" }}>Finished in 2m 41s · 4 findings, none blocking</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10 }}>
              {Object.entries(counts).map(([k, v]) => (
                <div key={k} style={{
                  padding: "8px 10px", borderRadius: 6,
                  background: v > 0 ? "var(--bg-sunken)" : "transparent",
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          {screenshots.length < 8 && (
            <button onClick={() => setScreenshots([...screenshots, screenshots.length + 1])} style={{
              width: 80, height: 142, borderRadius: 6,
              border: "1.5px dashed var(--border-default)", background: "var(--bg-sunken)",
              color: "var(--fg-tertiary)", display: "grid", placeItems: "center",
            }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ padding: "10px 14px",
        ...
        <window.Ico name="users" size={15} style={{ color: "var(--fg-tertiary)" }} />
        <div style={{ fontSize: 12.5 }}>
          <span className="mono" style={{ fontWeight: 600 }}>{subs.length}</span>{" "}
          <span style={{ color: "var(--fg-secondary)" }}>merchants are subscribed to this app</span>
          <span style={{ color: "var(--fg-tertiary)" }}> · <span className="mono">{subs.reduce((s, c) => s + c.terminals, 0)}</span> terminals total</span>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        {[
          ...
          <button key={opt.id} onClick={() => setScopeMode(opt.id)} style={{
            ...
            background: scopeMode === opt.id ? "var(--accent-50)" : "var(--bg-surface)",
            border: "1px solid",
            borderColor: scopeMode === opt.id ? "var(--accent-500)" : "var(--border-default)",
            borderRadius: 8,
            boxShadow: scopeMode === opt.id ? "0 0 0 3px oklch(58% 0.22 270 / 0.08)" : "none",
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{
                width: 14, height: 14, borderRadius: "50%",
                border: "1.5px solid",
                borderColor: scopeMode === opt.id ? "var(--accent-600)" : "var(--border-strong)",
                display: "grid", placeItems: "center",
              }}>
                {scopeMode === opt.id && <div style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--accent-600)" }} />}
              </div>
              <span style={{ fontSize: 13, fontWeight: 500 }}>{opt.label}</span>
            </div>
            <div style={{ marginTop: 8, fontSize: 11.5, color: "var(--fg-tertiary)", lineHeight: 1.45 }}>{opt.body}</div>
          </button>
        ))}
      </div>
        ...
        <window.Card padding={0} title={scopeMode === "whitelist" ? "Choose merchants to include" : "Choose merchants to exclude"}
          action={
            <span style={{ fontSize: 11.5, color: "var(--fg-tertiary)" }}>
              <span className="mono" style={{ color: "var(--fg-primary)", fontWeight: 500 }}>
            ...
            {filteredSubs.map((c, i) => {
              ...
              return (
                <button key={c.id} onClick={() => {
                  ...
                  borderBottom: i < filteredSubs.length - 1 ? "1px solid var(--border-subtle)" : "none",
                  background: on ? (scopeMode === "whitelist" ? "var(--success-bg)" : "var(--danger-bg)") : "transparent",
                }}>
                  <div style={{
                    width: 18, height: 18, borderRadius: 5,
                    background: on ? (scopeMode === "whitelist" ? "var(--success)" : "var(--danger)") : "var(--bg-surface)",
                    border: "1px solid",
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: "var(--fg-tertiary)" }}>{c.region} · <span className="mono">{c.terminals}</span> terminals</div>
                  </div>
      ...
      <window.Card title="Rollout strategy">
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <input type="range" min="10" max="100" step="10" value={rolloutPct}
            ...
            style={{ width: 220, accentColor: "var(--accent-600)" }} />
      ...
      <div style={{
        ...
        <window.Ico name="bolt" size={16} style={{ color: "var(--accent-700)" }} />
        <div style={{ flex: 1, fontSize: 12.5 }}>
          This release will reach{" "}
          <span className="mono" style={{ fontWeight: 600, fontSize: 14, color: "var(--accent-700)" }}>{Math.round(reachCount * rolloutPct / 100)}</span>{" "}
          of <span className="mono">{subscriberIds.length}</span> subscribed merchants in the first wave
          {rolloutPct < 100 && <span style={{ color: "var(--fg-tertiary)" }}> ({rolloutPct}% of {reachCount} scoped)</span>}.
        </div>
      </div>
    </div>
  );
...
// ─── Step 5: Review ───────────────────────────────────────
function StepReview({ app, parsed, counts, notes, scopeMode, reachCount, rolloutPct, subscriberIds, whitelist, blacklist }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <window.Card title="Summary">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
          ...
          <window.KV label="Scan result"     value={<span style={{ color: "var(--success)" }}>Clean — no blocking issues ({Object.values(counts).reduce((a, b) => a + b, 0)} findings)</span>} />
      ...
      <window.Card title="Release notes">
        <p style={{ margin: 0, fontSize: 12.5, lineHeight: 1.6, color: "var(--fg-secondary)", whiteSpace: "pre-wrap" }}>{notes || <span style={{ color: "var(--fg-tertiary)" }}>No release notes provided.</span>}</p>
      </window.Card>
      ...
      <div style={{
        ...
        borderRadius: 8,
        border: "1px solid", borderColor: "color-mix(in oklab, var(--warning) 30%, transparent)",
        display: "flex", alignItems: "flex-start", gap: 10, fontSize: 12, color: "var(--warning)",
      }}>
...
function SectionHead({ title, body }) {
  return (
    <div>
      ...
      {body && <p style={{ margin: "5px 0 0", fontSize: 12.5, color: "var(--fg-tertiary)", maxWidth: 640, lineHeight: 1.55 }}>{body}</p>}
    </div>
  );
}
