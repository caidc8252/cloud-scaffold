                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gap: 4, flex: 1 }}>
                    {iconOptions.map(o => (
                      <button key={o} onClick={() => setIconId(o)} style={{
                        ...
                        border: "2px solid",
                        borderColor: iconId === o ? "var(--accent-500)" : "transparent",
              ...
              <window.Field label="Package name" required
                ...
                <window.Input value={pkg} onChange={(e) => setPkg(e.target.value)}
                  ...
                  suffix={
                    ...
                    pkgState === "ok"       ? <window.Ico name="check" size={13} style={{ color: "var(--success)" }} stroke={2.5} /> :
                    pkgState === "taken"    ? <window.Ico name="x"     size={13} style={{ color: "var(--danger)"  }} stroke={2.5} /> :
                    pkgState === "invalid"  ? <window.Ico name="alert" size={13} style={{ color: "var(--danger)"  }} /> : null
                  } />
                {pkgState === "ok" && !isEdit && (
                  <div style={{ marginTop: 5, fontSize: 11, color: "var(--success)", display: "flex", alignItems: "center", gap: 5 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {["Payments", "Operations", "Customer", "Analytics", "Utility"].map(c => (
                    <button key={c} onClick={() => setCategory(c)} style={{
                      ...
                      background: category === c ? "var(--accent-50)" : "var(--bg-surface)",
                      border: "1px solid",
                      borderColor: category === c ? "var(--accent-500)" : "var(--border-default)",
                      color: category === c ? "var(--accent-700)" : "var(--fg-secondary)",
                ...
                return (
                  <button key={d.id} onClick={() => toggleDevice(d.id)} style={{
                    ...
                    <div style={{
                      ...
                      background: on ? "var(--accent-600)" : "transparent",
                      border: "1px solid",
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10.5, color: "var(--fg-tertiary)" }} className="tm-truncate">{d.blurb}</div>
                    </div>
                  </button>
                );
              })}
