...
function Sidebar({ route, navigate }) {
  ...
  return (
    <aside style={{
      width: 224, flexShrink: 0, background: "var(--color-bg-3)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 8px 14px" }}>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
          <span style={{ fontWeight: 700, fontSize: 14, letterSpacing: "-0.01em", color: "var(--color-primary-700)" }}>TOMS</span>
          <span style={{ fontSize: 10.5, fontFamily: "var(--font-family-mono)", color: "var(--color-text-tertiary)",
            padding: "1px 5px", border: "1px solid var(--color-border-default)", borderRadius: 3,
            letterSpacing: "0.04em", textTransform: "uppercase" }}>Carbon</span>
        </div>
        <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--color-text-tertiary)", padding: "2px 6px",
          background: "var(--color-bg-2)", border: "1px solid var(--color-border-subtle)", borderRadius: 4 }}>
          ISV
        </span>
      </div>
      ...
      <button style={{
        ...
        borderRadius: 7, color: "var(--color-text-primary)", fontSize: 12.5, textAlign: "left",
        boxShadow: "var(--shadow-1)",
      }}>
        <div style={{ width: 20, height: 20, borderRadius: 5, background: "var(--color-primary-700)",
          color: "white", display: "grid", placeItems: "center", fontSize: 10, fontWeight: 700 }}>A</div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 500, lineHeight: 1.1 }} className="truncate">Acme Software Inc.</div>
          <div style={{ fontSize: 10.5, color: "var(--color-text-tertiary)", marginTop: 1 }}>ISV · Publisher</div>
        </div>
            ...
            <a key={it.id} href="#" onClick={(e) => { e.preventDefault(); navigate({ screen: it.id }); }} style={{
              ...
              {it.count != null && (
                <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--color-text-tertiary)",
                  fontFamily: "var(--font-family-mono)" }}>{it.count}</span>
              )}
            </a>
          );
        })}
      ...
      <div style={{ marginTop: 18, padding: "6px 10px 4px", fontSize: 10.5,
        color: "var(--color-text-tertiary)", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 500 }}>
        Recent
      </div>
      <nav style={{ display: "flex", flexDirection: "column", gap: 1 }}>
        {[
          ...
          <a key={p.id} href="#" style={{
            ...
            <span style={{ width: 6, height: 6, borderRadius: "50%",
              background: p.state === "success" ? "var(--color-success-500)"
                       : p.state === "warning" ? "var(--color-warning-500)"
                       : "var(--color-text-tertiary)" }} />
            <span className="truncate" style={{ flex: 1 }}>{p.label}</span>
          </a>
        ))}
      </nav>
      ...
      <div style={{ marginTop: "auto", padding: "10px 8px",
        ...
        <div style={{ width: 22, height: 22, borderRadius: "50%",
          background: "linear-gradient(135deg, var(--color-primary-500), var(--color-accent-600))", color: "white",
          display: "grid", placeItems: "center", fontSize: 11, fontWeight: 600 }}>MH</div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 12, fontWeight: 500 }} className="truncate">Maya Hassan</div>
          <div style={{ fontSize: 10.5, color: "var(--color-text-tertiary)" }}>Publisher · Acme</div>
        </div>
        <button style={{ color: "var(--color-text-tertiary)", padding: 4 }} title="Account">
  ...
  return (
    <div style={{
          ...
          <React.Fragment key={i}>
            ...
            {c.href ? (
              <a href="#" onClick={(e) => { e.preventDefault(); c.onClick && c.onClick(); }}
                 style={{ color: i === crumbs.length - 1 ? "var(--color-text-secondary)" : "inherit",
                          textDecoration: "none", whiteSpace: "nowrap",
                          fontFamily: c.mono ? "var(--font-family-mono)" : undefined,
                          fontWeight: i === crumbs.length - 1 ? 500 : 400 }}>{c.label}</a>
            ) : (
              <span style={{ color: i === crumbs.length - 1 ? "var(--color-text-secondary)" : "inherit",
                             whiteSpace: "nowrap",
                             fontFamily: c.mono ? "var(--font-family-mono)" : undefined,
      ...
      <button style={{
        ...
        fontSize: 11.5, border: "1px solid var(--color-border-subtle)",
      ...
      <button style={{ padding: 4, color: "var(--color-text-tertiary)" }} title="Notifications">
  ...
  return (
    <button onClick={onClick} disabled={disabled} type={type} style={{
      ...
      background: ghost ? "transparent"
                ...
                : danger  ? "var(--color-bg-2)"
                : "var(--color-bg-2)",
      color: primary ? "var(--color-text-on-primary)"
...
function Pill({ tone = "neutral", children, dot, size = "md" }) {
  const toneMap = {
    success: { bg: "var(--color-success-50)", fg: "var(--color-success-500)" },
    warning: { bg: "var(--color-warning-50)", fg: "var(--color-warning-500)" },
    danger:  { bg: "var(--color-error-50)",  fg: "var(--color-error-500)"  },
    info:    { bg: "var(--color-info-50)",    fg: "var(--color-info-500)"    },
    accent:  { bg: "var(--color-primary-50)",  fg: "var(--color-primary-700)"},
    neutral: { bg: "var(--color-bg-3)",  fg: "var(--color-text-secondary)" },
  };
    ...
    <section style={{
      ...
      {(title || action) && (
        <div style={{
          ...
          {hint && <span style={{ fontSize: 11, color: "var(--color-text-tertiary)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{hint}</span>}
  ...
  return (
    <label style={{ display: "flex",
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 6, minWidth: layout === "row" ? 160 : undefined }}>
        ...
        {required && <span style={{ color: "var(--color-error-500)", fontSize: 11 }}>*</span>}
        {hint && layout === "stack" && <span style={{ fontSize: 11, color: "var(--color-text-tertiary)" }}>{hint}</span>}
      </div>
      <div style={{ flex: 1 }}>
        {children}
        {error && <div style={{ marginTop: 4, fontSize: 11, color: "var(--color-error-500)" }}>{error}</div>}
      </div>
    </label>
  );
}
...
function Input({ value, onChange, placeholder, mono, prefix, suffix, error, style, type = "text", ...rest }) {
  ...
  return (
    <div style={{
      ...
      border: "1px solid",
      borderColor: error ? "var(--color-error-500)" : focus ? "var(--color-primary-500)" : "var(--color-border-default)",
      borderRadius: 6,
      background: "var(--color-bg-2)",
      padding: "0 8px",
      boxShadow: focus ? "0 0 0 3px oklch(40% 0.14 262 / 0.15)" : "none",
      transition: "box-shadow .1s, border-color .1s",
      ...style,
    }}>
      {prefix && <span style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginRight: 6,
        fontFamily: mono ? "var(--font-family-mono)" : undefined }}>{prefix}</span>}
      <input value={value || ""} onChange={onChange} placeholder={placeholder} type={type}
        onFocus={() => setFocus(true)} onBlur={() => setFocus(false)}
        style={{
          flex: 1, border: 0, outline: 0, background: "transparent",
          padding: "6px 0", fontSize: 12.5,
          fontFamily: mono ? "var(--font-family-mono)" : "inherit", color: "var(--color-text-primary)",
        }} {...rest} />
      {suffix && <span style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginLeft: 6 }}>{suffix}</span>}
    </div>
  );
}
  ...
  return (
    <textarea value={value || ""} onChange={onChange} placeholder={placeholder} rows={rows}
      ...
      style={{
        ...
        border: "1px solid", borderColor: focus ? "var(--color-primary-500)" : "var(--color-border-default)",
        borderRadius: 6, background: "var(--color-bg-2)", color: "var(--color-text-primary)",
...
function KV({ label, value, mono, copy }) {
  return (
    <div style={{
      ...
      <div style={{ width: 150, fontSize: 11.5, color: "var(--color-text-tertiary)", flexShrink: 0 }}>{label}</div>
      <div style={{
        flex: 1, fontSize: 12.5, fontWeight: 450, color: "var(--color-text-primary)",
        fontFamily: mono ? "var(--font-family-mono)" : undefined,
        wordBreak: "break-word",
      }}>{value}</div>
      {copy && (
        <button style={{ color: "var(--color-text-tertiary)", padding: 2, opacity: 0.6 }} title="Copy">
...
function Empty({ title, body, icon = "doc", action }) {
  return (
    <div style={{
      ...
      {body && <div style={{ fontSize: 12, color: "var(--color-text-tertiary)", maxWidth: 360 }}>{body}</div>}
        ...
        {(title || subtitle) && (
          <header style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              {subtitle && <div style={{ marginTop: 3, fontSize: 12, color: "var(--color-text-tertiary)" }}>{subtitle}</div>}
            </div>
            <button onClick={onClose} style={{ padding: 6, color: "var(--color-text-tertiary)", borderRadius: 6 }} title="Close">
              <Ico name="x" size={15} />
            </button>
          </header>
        )}
...
function Toast({ toast, onClose }) {
  ...
  const toneColor = tone === "success" ? "var(--color-success-500)" : tone === "danger" ? "var(--color-error-500)" : "var(--color-primary-500)";
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: tone === "success" ? "var(--color-success-50)" : tone === "danger" ? "var(--color-error-50)" : "var(--color-primary-50)",
      ...
      <button onClick={onClose} style={{ color: "var(--color-text-tertiary)", padding: 4 }}>
        <Ico name="x" size={13} />
      </button>
...
function SeverityBar({ counts, height = 6 }) {
  ...
  if (!total) {
    return (
      <div style={{ height, borderRadius: 999, background: "var(--color-success-50)",
