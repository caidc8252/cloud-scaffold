...
const OrderList = ({ orders, onOpen, onNew }) => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      inTransit: byStatus('Shipped') + byStatus('Partially complete'),
    };
  }, [orders]);
  ...
  return (
    <div className="page">
      ...
      <div className="stats" style={{ gridTemplateColumns: 'repeat(4, minmax(0, 1fr))' }}>
        ...
        <button type="button" className={`stat is-clickable ${status === 'In transit' ? 'is-active' : ''}`} onClick={() => { setStatus('In transit'); setPage(1); }}>
          <div className="stat__label">In transit</div>
          <div className="stat__val" style={{ color: 'var(--color-success-700)' }}>{stats.inTransit}</div>
          <div className="stat__delta">shipped or partially delivered</div>
          <div className="stat__action">Filter <Icon name="chevR" size={9}/></div>
        </button>
      </div>
