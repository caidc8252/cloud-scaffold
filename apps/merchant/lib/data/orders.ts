export type OrderStatus = 'Awaiting payment' | 'Awaiting shipment' | 'Shipped' | 'Partially complete' | 'Complete'
...
export const DEVICE_MODELS: DeviceModel[] = [
  { id: 'm-n950', name: 'N950', family: 'Smart Android', desc: 'Smart Android terminal, 5.5" touch, NFC + magstripe.', unitPrice: 459, types: ['Semi-integration', 'Stand-alone'], image: null },
  { id: 'm-s30', name: 'S30', family: 'Countertop', desc: 'Compact countertop terminal, Ethernet + dial-up.', unitPrice: 269, types: ['Semi-integration', 'Stand-alone'], image: null },
  { id: 'm-s60', name: 'S60', family: 'Smart POS', desc: 'Dual-screen smart POS, built-in thermal printer.', unitPrice: 689, types: ['Semi-integration', 'Stand-alone'], image: null },
  { id: 'm-s90', name: 'S90', family: 'Portable', desc: 'Portable wireless terminal, 4G + Wi-Fi + Bluetooth.', unitPrice: 389, types: ['Semi-integration', 'Stand-alone'], image: null },
  { id: 'm-n750', name: 'N750', family: 'Mobile', desc: 'Slim mobile Android handheld, 4G + NFC.', unitPrice: 319, types: ['Semi-integration', 'Stand-alone'], image: null },
  { id: 'm-x800', name: 'X800', family: 'Kiosk', desc: 'Unattended kiosk module, QR + NFC, IP54 sealed.', unitPrice: 1180, types: ['Semi-integration'], image: null },
]

export const ORDER_STATUSES: OrderStatus[] = ['Awaiting payment', 'Awaiting shipment', 'Shipped', 'Partially complete', 'Complete']

export const ORDER_STATUS_TONE: Record<string, 'warning' | 'info' | 'success' | 'neutral'> = {
...
export function fakeSerialNumber(prefix: string, n: number): string {
  return `${prefix}-${String(n).padStart(8, '0')}`
}

export function fakeActivationCode(seed: number = 1): string {
  return String(100000 + ((seed * 7919) % 900000)).padStart(6, '0')
}

export function emptyDevices(qty: number): OrderDevice[] {
  void qty
  return []
}

export function populatedDevices(qty: number, prefix: string, start: number = 1): OrderDevice[] {
  return Array.from({ length: qty }, (_, i) => ({
    sn: fakeSerialNumber(prefix, start + i),
    code: fakeActivationCode(start + i),
    type: '',
  }))
}

export function orderSubtotal(order: { items: { unitPrice: number; qty: number }[] }): number {
  return order.items.reduce((n, i) => n + i.unitPrice * i.qty, 0)
}

export function orderTotal(order: { items: { unitPrice: number; qty: number }[]; discountPct: number }): number {
  return Math.round(orderSubtotal(order) * (1 - (order.discountPct || 0) / 100) * 100) / 100
}

export function orderQty(order: { items: { qty: number }[] }): number {
  return order.items.reduce((n, i) => n + i.qty, 0)
}

export function deviceProgress(order: { items: { qty: number; devices: OrderDevice[] }[] }): { done: number; total: number } {
  let done = 0, total = 0
...
export const SEED_ORDERS: Order[] = [
  ...
  {
    id: 'o-2026-0184', number: 'SO-2026-0184',
    customerId: 'c-001', customerName: 'Northwind Commerce',
    createdAt: '2026-05-09T14:22:00Z', createdBy: 'jordan.d@carbon',
    status: 'Awaiting payment',
    items: [
      { id: 'li-1', modelId: 'm-n950', modelName: 'N950', unitPrice: 459, qty: 12, type: 'Semi-integration', devices: [] },
      { id: 'li-2', modelId: 'm-s30', modelName: 'S30', unitPrice: 269, qty: 8, type: 'Stand-alone', devices: [] },
    ],
    discountPct: 5,
  ...
  {
    id: 'o-2026-0181', number: 'SO-2026-0181',
    customerId: 'c-002', customerName: 'Helios Payments',
    createdAt: '2026-05-07T11:08:00Z', createdBy: 'jordan.d@carbon',
    status: 'Awaiting shipment',
    items: [
      { id: 'li-1', modelId: 'm-n750', modelName: 'N750', unitPrice: 319, qty: 4, type: 'Stand-alone', devices: [] },
      { id: 'li-2', modelId: 'm-s60', modelName: 'S60', unitPrice: 689, qty: 2, type: 'Semi-integration', devices: [] },
    ],
    discountPct: 100,
  ...
  {
    id: 'o-2026-0177', number: 'SO-2026-0177',
    customerId: 'c-003', customerName: 'Brightleaf Retail',
    createdAt: '2026-05-04T16:40:00Z', createdBy: 'admin@carbon',
    status: 'Awaiting shipment',
    items: [
      {
  ...
  {
    id: 'o-2026-0152', number: 'SO-2026-0152',
    customerId: 'c-004', customerName: 'Vanta Software',
    createdAt: '2026-04-21T09:15:00Z', createdBy: 'admin@carbon',
    status: 'Shipped',
    items: [
  ...
  {
    id: 'o-2026-0140', number: 'SO-2026-0140',
    customerId: 'c-001', customerName: 'Northwind Commerce',
    createdAt: '2026-04-10T13:00:00Z', createdBy: 'jordan.d@carbon',
    ...
    items: [
      ...
      { id: 'li-2', modelId: 'm-x800', modelName: 'X800', unitPrice: 1180, qty: 2, type: 'Semi-integration', devices: [], shipped: false },
    ],
    discountPct: 0,
  ...
  // 6) Complete
  {
    id: 'o-2026-0098', number: 'SO-2026-0098',
    customerId: 'c-003', customerName: 'Brightleaf Retail',
    createdAt: '2026-03-18T10:30:00Z', createdBy: 'admin@carbon',
    status: 'Complete',
    items: [
